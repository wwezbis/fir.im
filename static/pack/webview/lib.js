...
$(".ng-binding").attr("disabled","disabled").text("生成中...");xhr.open("GET",in_path+"source/pack/webview/ajax.php?ac=webview&title="+escape($("#in_title").val())+"&url="+$("#in_url").val()+"&bcolor="+$("#in_b_color").val()+"&tcolor="+$("#in_t_color").val()+"&aicon="+$("#preview_a_icon img")[0].src+"&limage="+$("#preview_l_image img")[0].src,true);xhr.onreadystatechange=function(){if(xhr.readyState==4){if(xhr.status==200){if(xhr.responseText=="return_0"){$(".ng-binding").text("请先登录");}else if(xhr.responseText=="return_1"){$(".ng-binding").text("点数不足");}else{$(".ng-binding").text("上传中...");ReturnValue(eval("("+xhr.responseText+")"));}}else{$(".ng-binding").text("通讯异常");}}};xhr.send(null);}
function ReturnValue(response){var xhr=new XMLHttpRequest();xhr.onreadystatechange=function(){processAJAX();};xhr.open("GET",in_path+"source/pack/upload/index-"+response.extension+".php?time="+response.time+"&size="+response.size+"&id=0",true);xhr.send(null);function processAJAX(){if(xhr.readyState==4){if(xhr.status==200){if(xhr.responseText==-1){$(".ng-binding").text("请先登录");}else if(xhr.responseText==-2){$(".ng-binding").text("非法操作");}else if(xhr.responseText==1){location.href=in_path+"index.php/home";}else{$(".ng-binding").text("内部错误");}}}}}